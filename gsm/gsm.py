...
test = load_dataset("openai/gsm8k", "main")
...
def extract_ground_truth_number(gt_answer: str) -> str:
    match = re.search(r"####\s*([0-9]+(?:\.[0-9]+)?)", gt_answer)
    if match:
...
def main():
    ...
    for data in questions[:1]:
        ...
        if answer is not None:
          gt_final = extract_ground_truth_number(answer)
